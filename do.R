model <- function( killed_dfx,train ) {
  
  require(rpart)
  
  train$word <- as.character(train$word)
  killed_dfx$word <- as.character(killed_dfx$word)
  train <- feature(killed_dfx,train)
  train1 <- train[,c("hashtag","titlenum","articlenum","exclamations","question","link","label")]
  train2 <- train[,(ncol(train)-nrow(killed_dfx) + 1):ncol(train)]
  train <- cbind( train1,train2)
  
  linearmodel <- rpart( as.factor(label)~.,data = train)  # polynomial radial basis sigmoid linear
...
feature <- function(killed_dfx,data) {
  i = 1 
  killed_dfx$word <- as.character(killed_dfx$word)
  data$word <- as.character(  data$word)
  while( i <= nrow(killed_dfx)) {
    data[,ncol(data)+1] <- 0
    colnames(data)[ncol(data)]  <- killed_dfx$word[i] 
    j = 1 
    
    while( nrow(data) >= j ) {
      count <- 0 
      while( !is.na(regexpr(killed_dfx$word[i],data$word[j]) ) &&regexpr(killed_dfx$word[i],data$word[j]) != -1  ) {
        data$word[j] <- sub(killed_dfx$word[i],"",data$word[j])
        count <- count + 1 
      }
      data[j,ncol(data)] <- count
      j = j  + 1 
    }
